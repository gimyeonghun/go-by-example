...
// A Go string is a read-only slice of bytes. The language and the standard library treats strings specially - as containers of text encoded in UTF-8. 
...
func main() {
  ...
  // To count how many runes are in a string, we can use the `utf-8` package. The run-time of RuneCountInString depends on the size of the string, because it has to decode each UTF-8 rune sequentially. 
